...
//! (The De7 ack-confirmation assertion moved in-process in project 3, P7 —
//! `deliver__should_record_the_recipients_own_ack_as_a_confirmation`.)
...
async fn outbox__should_queue_while_the_relay_is_down_and_flush_when_it_returns() {
    // REAL-NETWORK SMOKE (P7, transport.md §8): a relay restarting at the
    // same dial string (persisted identity, stable port) — queue while down,
    // deliver on return.
