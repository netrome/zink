...
| Timing entropy | **Behind a port** (broken-window fix, 2026-08-15: `subscribe`'s reconnect jitter drew `OsRng` directly in domain code, through a float detour ending in panic-capable `Duration::mul_f64` and an unchecked `backoff * 2`). `ports/rng.rs`: `Draw` — `draw(bound) → u64` uniform in `[0, bound)`, for timing decisions only; `adapters/system_rng.rs`: `SystemRng`; fourth `Client` parameter (`R: Draw = SystemRng`), same one-param-per-port rule, edges unchanged. The policy is the pure `recv::jittered` — integer-millisecond, saturating, band `[backoff/2, 3·backoff/2)` pinned by `TestDraw` extremes (the old float band silently included both endpoints). **Crypto randomness is deliberately not this port**: sealing/keygen keep passing `CryptoRngCore` explicitly — a scriptable crypto-entropy port would be a footgun, and `zink-protocol` already owns that seam. The adapters audit line now reads "no iroh type, real clock, or timing entropy outside `adapters/`". → ADR 0004 addendum at close. |
