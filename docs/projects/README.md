...
| 7 | Profile pages — one person page over a key cluster | 🚧 in progress (scoped 2026-08-20) | [7-profile-pages/tracker.md](./7-profile-pages/tracker.md) |
| 8 | Shared conversation names — my lens travels, theirs renders | 💡 proposed (2026-08-20) | [8-shared-conversation-names/tracker.md](./8-shared-conversation-names/tracker.md) |
| 9 | Storage port — state I/O joins time and transport | 💡 proposed (2026-08-20) | [9-storage-port/tracker.md](./9-storage-port/tracker.md) |
...
after it, so the split lands on 3's clean seams. Projects 5–6 run
concurrently on separate branches: 5 is client/delivery work, 6 is app-layer
UX; their shared surfaces (`chat.rs`, the Person view) are disjoint in
content, so rebases are expected to be textual noise only.*
